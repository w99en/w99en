...
def brute_force(points: List[Point]) -> Tuple[float, Point, Point]:
    """蛮力法 O(n^2)"""
    min_dist_sq = float('inf')  # 使用距离的平方来避免开方
    result = (None, None)
    n = len(points)
    for i in range(n):
...
def closest_pair(points: List[Point]) -> Tuple[float, Point, Point]:
    ...
    def recursive_helper(px, py):
        ...
        min_strip_sq = d  # 存储的是距离的平方
        for i in range(len(strip)):
            for j in range(i + 1, min(i + 7, len(strip))):
                # 计算距离的平方
                dist_sq = (strip[i][0] - strip[j][0]) ** 2 + (strip[i][1] - strip[j][1]) ** 2
                if dist_sq < min_strip_sq:
                    min_strip_sq = dist_sq
                    min_pair = (strip[i], strip[j])

        return (min_strip_sq, *min_pair) if min_strip_sq < d else (d, *min_pair)
...
def run_test(sizes: List[int], run_brute_force: bool):
    ...
    for size in sizes:
        ...
        # 运行分治法
        start = time.time()
        dc_dist_sq, dc_p1, dc_p2 = closest_pair(points)
        ...
        if run_brute_force:
            start = time.time()
            bf_dist_sq, bf_p1, bf_p2 = brute_force(points)
            bf_time = time.time() - start
            assert abs(dc_dist_sq - bf_dist_sq) < 1e-6, "结果不一致"
            print(f"最小距离: {math.sqrt(bf_dist_sq):.2f}, 点对: ({bf_p1[0]:.3f}, {bf_p1[1]:.3f}) 和 ({bf_p2[0]:.3f}, {bf_p2[1]:.3f})")

        print(f"最小距离: {math.sqrt(dc_dist_sq):.2f}, 点对: ({dc_p1[0]:.3f}, {dc_p1[1]:.3f}) 和 ({dc_p2[0]:.3f}, {dc_p2[1]:.3f})")

        results.append((size, bf_time, dc_time, dc_dist_sq, dc_p1, dc_p2))
        ...
        if size <= 1000:  # 小样本时画出结果图
            plot_points_and_min_dist(points, dc_p1, dc_p2, dc_dist_sq)
    return results

def plot_points_and_min_dist(points: List[Point], p1: Point, p2: Point, dist_sq: float):
    """绘制点和最小距离点对的连线"""
    ...
    # 绘制最小距离的点对
    plt.plot([p1[0], p2[0]], [p1[1], p2[1]], 'r-', lw=2, label=f'Min Dist: {math.sqrt(dist_sq):.2f}')
...
if __name__ == "__main__":
    ...
    for size, bf, dc, dc_dist_sq, dc_p1, dc_p2 in all_results:
        if bf is not None:
            print(f"{size}\t{bf:.2f}s\t\t{dc:.2f}s\t最小距离: {math.sqrt(dc_dist_sq):.2f}, 点对: ({dc_p1[0]:.3f}, {dc_p1[1]:.3f}) 和 ({dc_p2[0]:.3f}, {dc_p2[1]:.3f})")
        else:
            print(f"{size}\tN/A\t\t{dc:.2f}s\t最小距离: {math.sqrt(dc_dist_sq):.2f}, 点对: ({dc_p1[0]:.3f}, {dc_p1[1]:.3f}) 和 ({dc_p2[0]:.3f}, {dc_p2[1]:.3f})")
