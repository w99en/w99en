...
# def plot_results(results):
    ...
    # plt.subplot(1, 2, 1)
    # plt.plot(bf_sizes, bf_times, 'ro-', label='Brute Force')
    # plt.plot(sizes, dc_times, 'bo-', label='Divide and Conquer')
    ...
    # plt.subplot(1, 2, 2)
    # plt.loglog(bf_sizes, bf_times, 'ro-', label='Brute Force')
    # plt.loglog(sizes, dc_times, 'bo-', label='Divide and Conquer')
...
def plot_results(results):
    ...
    plt.subplot(1, 2, 1)
    if len(bf_times) > 0:
        plt.plot(bf_sizes, bf_times, 'ro-', label='Brute Force (实际)')
        plt.plot(bf_sizes, theoretical_bf, 'r--', label='Brute Force (理论O(n²))')
    plt.plot(sizes, dc_times, 'bo-', label='Divide and Conquer (实际)')
    plt.plot(sizes, theoretical_dc, 'b--', label='Divide and Conquer (理论O(n log n))')
    plt.xlabel('Problem Size')
    plt.ylabel('Time (s)')
    plt.title('实际性能与理论曲线对比')
    plt.legend()
    plt.grid(True)
    ...
    plt.subplot(1, 2, 2)
    if len(bf_times) > 0:
        plt.loglog(bf_sizes, bf_times, 'ro-', label='Brute Force (实际)')
        plt.loglog(bf_sizes, theoretical_bf, 'r--', label='Brute Force (理论O(n²))')
    plt.loglog(sizes, dc_times, 'bo-', label='Divide and Conquer (实际)')
    plt.loglog(sizes, theoretical_dc, 'b--', label='Divide and Conquer (理论O(n log n))')
    plt.xlabel('Problem Size (log)')
    plt.ylabel('Time (s) (log)')
    plt.title('对数坐标下的对比')
    plt.legend()
