...
class GameStateData:
    ...
    def initialize(self, layout, numGhostAgents):
        ...
        self.agentStates = []
        numGhosts = 0
    #     pacmanAdded = False  # 用于跟踪是否已添加Pacman  
    #     for isPacman, pos in layout.agentPositions:  
    #         if isPacman:  
    #         # 总是添加Pacman代理  
    #             self.agentStates.append(AgentState(Configuration(pos, Directions.STOP), isPacman))  
    #             pacmanAdded = True  
    #         elif not pacmanAdded and numGhosts < numGhostAgents:  
    #         # 只有在没有添加Pacman且未达到幽灵最大数量时才添加幽灵代理  
    #             numGhosts += 1  
    #             self.agentStates.append(AgentState(Configuration(pos, Directions.STOP), isPacman))  
  
    # # 如果layout中没有Pacman，但我们需要一个，则手动添加一个  
    #     if not pacmanAdded:  
    #         # 这里可能需要一个默认的起始位置，或者抛出一个错误  
    #         default_pacman_pos = (0,0) # 需要您定义默认的Pacman位置  
    #         self.agentStates.append(AgentState(Configuration(default_pacman_pos, Directions.STOP), True))  
